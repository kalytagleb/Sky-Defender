...
import objects.enemies.AbstractEnemy;
import objects.enemies.BasicEnemy;
import objects.GameObject;
import objects.Player;
import objects.enemies.FastEnemy;
import objects.enemies.TankEnemy;
import objects.factory.FlameFactory;
import objects.factory.LaserFactory;
import objects.factory.RocketFactory;
import objects.factory.WeaponFactory;
...
import static logic_units.Collision.intersects;
...
public class PanelGame extends JComponent {
    ...
    /** Thread used to drive the main game loop */
    ...
    /** Flag for main menu screen */
    private boolean inMainMenu = true;

    /** Flag for manual screen */
    private boolean showingManual = false;

    /** Flags for wave state */
    ...
    /** Time stamp for wave text display */
    private long waveStartTime;

    /** Desired frames per second */
    private final int FPS = 60;

    /** Nanoseconds per frame (based on FPS) */
    private final int TARGET_TIME = 1000000000 / FPS;

    /** Score counter */
    private int score = 0;

    /** Counter to control weapon firing rate */
    private int shotTime;

    /** Keyboard input handler */
    private Key key;

    /** Button areas for manual screen navigation */
    private Rectangle manualButton = new Rectangle();
    private Rectangle manualBackButton = new Rectangle();

    /** Player object */
    private Player player;

    /** List of all enemies on screen */
    private List<AbstractEnemy> enemies = new CopyOnWriteArrayList<>();

    /** List of all active weapons on screen */
    private List<Weapon> weapons = new CopyOnWriteArrayList<>();
    ...
    private void initKeyboard() {
        ...
        new Thread(() -> {
            ...
            while (start) {
                ...
                if (key.isKey_left()) angle -= rotateSpeed;
                if (key.isKey_right()) angle += rotateSpeed;
                ...
                enemies.removeIf(enemy -> {
                    enemy.update();
                    return enemy.check(width, height);
                });
